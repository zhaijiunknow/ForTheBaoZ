...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public bool ResolveActionAndBinding(out InputAction action, out int bindingIndex)
        {
            ...
            string action_in_asset = action.ToString();
        ...
        private bool CheckDuplicateBinding(InputAction action, int index, bool isComposite = false)
        {
            InputBinding newBingding = action.bindings[index];
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                if (binding.action == newBingding.action)
                {
                    continue;
                }
                if (binding.effectivePath == newBingding.effectivePath)
                {
            ...
            if (isComposite)
            {
                ...
                {
                    if (action.bindings[i].effectivePath == newBingding.overridePath)
                    {
                        Debug.Log("duplicate binding");
                        return true;
                    }
